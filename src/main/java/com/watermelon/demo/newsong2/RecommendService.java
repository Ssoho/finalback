...
//        boolean userExists = recommendRepository.existsByUser_id(userId);
        ...
        if (data.isPresent()) { // 매칭되는 데이터가 있다면, 
            ...
            String recommended_songs = existingRecommendation.getRecommended_songs2();
    ...
    ResponseEntity<String> fetchRecommendationsFromFlask(Long userId, Optional<Recommend> data) throws JsonMappingException, JsonProcessingException {
    	// flask와 소통을 위한 정의
        String flaskAppUrl = "http://localhost:5000/recommend-songs2/" + userId;
        ...
        if (responseEntity.getStatusCode().is2xxSuccessful()) { 
            ...
            List<Long> recommendedSongs = objectMapper.readValue((String) responseMap.get("recommended_songs2"), new TypeReference<List<Long>>() {}); // String list -> list<Long>으로 변환
