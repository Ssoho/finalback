...
//    public ResponseEntity<String> updateSong(Long userId, @RequestBody Optional<Recommend> recommend) throws JsonMappingException, JsonProcessingException{
//        Optional<Recommend> data = recommendRepository.findByUser_id(userId);
//        if (data.isPresent()) {
//            Recommend update_songs = data.get();
//            update_songs.setSong_ids(update_songs.getSong_ids());
//            Recommend updatedRecommend = recommendRepository.save(update_songs);
//            recommend = Optional.ofNullable(updatedRecommend);
//            return fetchRecommendationsFromFlask(userId, recommend);
////            return new ResponseEntity<>(updatedRecommend, HttpStatus.OK);
//        } else {
//            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
//        }
//        
...
//        boolean userExists = recommendRepository.existsByUser_id(userId);
        ...
        if (data.isPresent()) { // 매칭되는 데이터가 있다면, 
            ...
            String recommended_songs = existingRecommendation.getRecommended_songs();
    ...
    ResponseEntity<String> fetchRecommendationsFromFlask(Long userId, Optional<Recommend> data) throws JsonMappingException, JsonProcessingException {
    	// flask와 소통을 위한 정의
        String flaskAppUrl = "http://localhost:5000/recommend-songs/" + userId;
        ...
        if (responseEntity.getStatusCode().is2xxSuccessful()) { 
            ...
            List<Long> recommendedSongs = objectMapper.readValue((String) responseMap.get("recommended_songs"), new TypeReference<List<Long>>() {}); // String list -> list<Long>으로 변환
